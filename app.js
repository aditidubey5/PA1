...
function renderQuestion() {
    ...
    area.innerHTML = `
        <div class="q-card fade-in">
            <p class="q-text">${currentIdx + 1}. ${qText}</p>
            <div class="opt-group">
                <span>Disagree</span>
                ${[1, 2, 3, 4, 5].map(v => `
...
function generateLongReport(name, avg) {
    const isHigh = avg > 3;
    const execSummary = isHigh ? "High Agency Profile: Proactive and decisive." : "High Integrity Profile: Methodical and stable.";
    const deepDive = `
        <div class="report-section"><h3>Core Architecture</h3><p>Detailed analysis of behavioral drivers...</p></div>
        <div class="report-section"><h3>Communication</h3><p>Interpersonal dynamics and leadership style...</p></div>
    `;
...
function calculateReport() {
    ...
    document.getElementById('report-wrapper').innerHTML = `
        <div class="report-header-dark">
            <small>Official People Assets Report</small>
            <h1>${report.title}</h1>
        </div>
        <div class="report-body">
            <p class="summary-text">${report.summary}</p>
            ${report.deepDive}
            <button class="btn-primary" onclick="showPage('tests')">Back to Assessments</button>
        </div>`;
    showPage('report');

    // Background Email (No waiting)
    if (emailAddr) {
