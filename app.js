...
async function sendReportEmail() {
  ...
  try {
    // 1. Capture the exact visual report from the screen
    const reportHTML = document.getElementById("report-page-content").innerHTML;

    // 2. Prepare the parameters for EmailJS
    const templateParams = {
      to_email: email,
      test_name: currentTest.title,
      // We send the HTML code. Because it's text, it is very small.
      report_html: `
        <div style="background-color: #f8fafc; padding: 20px; font-family: sans-serif;">
          <div style="max-width: 600px; margin: auto; background: white; border-radius: 20px; border: 1px solid #e2e8f0; overflow: hidden;">
            ${reportHTML}
          </div>
          <div style="text-align: center; margin-top: 20px; color: #94a3b8; font-size: 12px;">
            Generated by People Assets • <a href="https://aditidubey5.github.io/PA1">Visit Website</a>
          </div>
        </div>
      `
    };

    // 3. Send using your existing EmailJS IDs
    await emailjs.send("service_u11zlzf", "template_zpcklyu", templateParams);

    // 4. Success UI
    statusEl.style.display = "block";
    statusEl.style.color   = "#10b981";
    statusEl.textContent   = "✓ Report successfully sent to " + email;
    sendBtn.textContent    = "Sent ✓";
    emailInput.value       = "";
    
    alert("Success! Your " + currentTest.title + " report has been delivered to " + email);
