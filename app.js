...
function showPage(page, testId = null, shouldPush = true) {
    ...
    if (shouldPush) {
        const path = page === 'test-landing' && testId ? `/${testId}` : page === 'home' ? '/' : `/${page}`;
...
function initRouter() {
    ...
    else if (["tests", "coaching", "profile"].includes(path)) showPage(path, null, false);
...
function renderTestGrid() {
    ...
    document.querySelectorAll('.filter-pill').forEach(pill => {
        pill.classList.toggle('active', pill.getAttribute('onclick').includes(`'${activeCategory}'`));
    });
...
function handleModalBackdropClick(e) { if (e.target.id === "method-modal") closeModal(); }
...
function submitCoachingForm(e) {
    ...
    emailjs.send("service_u11zlzf", "template_zpcklyu", {
        ...
        status.style.display = "block"; status.style.color = "green"; status.textContent = "Request sent!";
        btn.textContent = "Sent";
    }).catch(() => {
        status.style.display = "block"; status.style.color = "red"; status.textContent = "Failed to send.";
        btn.disabled = false; btn.textContent = "Request Coaching";
    });
}
