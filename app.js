...
async function sendReportEmail() {
  ...
  sendBtn.textContent = "Cleaning Report...";
  statusEl.style.display = "none";

  try {
    // 1. CLONE THE REPORT TO CLEAN IT
    // We create a copy so we don't change what the user sees on their screen
    const originalReport = document.getElementById("report-page-content");
    const reportClone = originalReport.cloneNode(true);

    // 2. REMOVE UNPROFESSIONAL ELEMENTS FROM THE EMAIL VERSION
    // We remove the buttons, the email box, and the coaching request box
    const selectorsToRemove = [
      '.report-actions',      // The bottom buttons
      '#email-report-box',    // The signup box itself
      'button',               // Any remaining buttons
      '.coaching-card'        // Any coaching forms if present
    ];

    selectorsToRemove.forEach(selector => {
      reportClone.querySelectorAll(selector).forEach(el => el.remove());
    });

    // 3. FIX THE "BLANK PAGE" / EXTRA SPACING
    // We force the clean HTML to have no top margins or hidden overflows
    ...
    const templateParams = {
      ...
      // We wrap the cleaned HTML in a professional container
      report_html: `
        <div style="font-family: sans-serif; color: #1e293b; line-height: 1.5;">
          <div style="max-width: 600px; margin: 0 auto; background: white; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden;">
            <div style="padding: 30px;">
              ${cleanHTML}
            </div>
            <div style="background: #f8fafc; padding: 20px; text-align: center; border-top: 1px solid #e2e8f0;">
              <p style="margin: 0; font-size: 12px; color: #94a3b8;">
                This professional assessment was generated by <strong>People Assets</strong>.
                <br>
                <a href="https://aditidubey5.github.io/PA1" style="color: #6366f1; text-decoration: none;">View Website</a>
              </p>
            </div>
          </div>
        </div>
    ...
    statusEl.textContent = "✓ Clean report sent to " + email;
    sendBtn.textContent = "Sent ✓";
    emailInput.value = "";
    
    alert("Success! A clean, professional version of your report has been sent to " + email);
