...
async function renderProfilePage() {
  ...
  section.innerHTML = buildProfileHTML(
    user,
    userName,
    userAvatar,
    userEmail,
  ...
  if (allResults.length > 0) {
    ...
    if (el)
      el.innerHTML = `<p style="color:#64748b;text-align:center;padding:20px 0;">Take your first assessment to unlock your AI Profile Summary.</p>`;
  }
}
...
function buildProfileHTML(user, userName, userAvatar, userEmail, results) {
  ...
  const avgScore =
    totalTests > 0
      ? Math.round(
          results.reduce((a, r) => a + (r.overall_score || 0), 0) / totalTests,
        )
      : null;
